...
class CODa2KITTI(object):
    ...
    def set_filename_by_prefix(modality, sensor_name, trajectory, frame):
        if "2d_rect"==modality:
            filetype = "png"  # CODa uses PNG format
        elif "2d_bbox"==modality:
            filetype = "txt"
        elif "3d_raw"==modality or "3d_comp"==modality:
            filetype = "bin"
    ...
    def save_image(self, traj, src_img_path, cam_id, frame_idx, file_idx):
        """Parse and save the images in png format. CODa dataset uses PNG format.
        ...
        kitti_img_path = f'{self.image_save_dir}{str(cam_id)}/' + \
                f'{str(traj).zfill(2)}{str(frame_idx).zfill(5)}.png'
    ...
    def save_label(self, traj, cam_id, frame_idx, file_idx):
        ...
        # CODa dataset doesn't include 2D bboxes - use dummy values for KITTI format
        # 2D bbox is not used for LiDAR-only 3D detection models like PointPillar
        twod_anno_dict = None
        ...
        for anno_idx, tred_anno in enumerate(anno_dict["3dbbox"]):
            ...
            # Use dummy 2D bbox values (not used for LiDAR-only models)
            bounding_box = [0.0, 0.0, 50.0, 50.0]
